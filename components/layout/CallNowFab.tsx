...
export default function CallNowFab({ href }: Props) {
  ...
  return (
    <div className="lux-call-fab pointer-events-none fixed bottom-[max(1.25rem,env(safe-area-inset-bottom,0px))] right-[max(1.25rem,env(safe-area-inset-right,0px))] z-50 sm:bottom-[max(1.5rem,env(safe-area-inset-bottom,0px))] sm:right-[max(1.5rem,env(safe-area-inset-right,0px))]">
      <a
        href={href}
        className="lux-call-fab__link pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full border border-white/20 bg-theme-bg/95 text-theme-on-bg-muted shadow-lg ring-1 ring-white/10 backdrop-blur-md transition-transform duration-300 hover:scale-[1.06] active:scale-[0.97] sm:h-15 sm:w-15"
        aria-label="Call now"
      >
